...
export default function CategoryBuilder({ 
  ...
  const getComponentTitle = (type: string) => {
    const titles = {
      ...
      'price-calculator': 'Калькулятор цен',
      'image-gallery': 'Галерея изображений'
    };
    return titles[type] || 'Компонент';
  };
  ...
  const getDefaultConfig = (type: string) => {
    const configs = {
      ...
      'search-bar': { placeholder: 'Поиск товаров...', showSuggestions: true },
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8">
        <Card className="w-full max-w-md">
          <div className="p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">Информация о категории</h2>
      ...
      <div className="flex h-full">
        ...
        <div className="w-80 bg-white border-r border-gray-200 flex flex-col">
          <div className="p-4 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900">Компоненты</h3>
            <p className="text-sm text-gray-600">Добавьте компоненты на страницу</p>
          </div>

          <div className="flex-1 overflow-y-auto p-4">
            <div className="space-y-3">
              {[
                { type: 'product-grid', icon: '', title: 'Сетка товаров', desc: 'Отображение товаров в сетке' },
                { type: 'filter-panel', icon: '', title: 'Фильтры', desc: 'Панель фильтрации товаров' },
                { type: 'search-bar', icon: '', title: 'Поиск', desc: 'Строка поиска товаров' },
                { type: 'cart-summary', icon: '', title: 'Корзина', desc: 'Сводка корзины' },
                { type: 'price-calculator', icon: '', title: 'Калькулятор', desc: 'Расчет стоимости' },
                { type: 'image-gallery', icon: '', title: 'Галерея', desc: 'Галерея изображений' }
              ].map((component) => (
                <div
                  key={component.type}
          ...
          <div className="bg-white border-b border-gray-200 px-4 py-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <h3 className="text-lg font-semibold text-gray-900">Конструктор страницы</h3>
                <span className="text-sm text-gray-500">Компонентов: {components.length}</span>
              </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <Card className="w-full max-w-md">
          <div className="p-6 text-center">
            <div className="text-6xl mb-4"></div>
            <h2 className="text-xl font-semibold text-gray-900 mb-2">Категория создана!</h2>
            <p className="text-gray-600 mb-6">
              Ваша категория успешно создана и настроена.
            </p>
