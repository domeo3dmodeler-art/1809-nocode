...
export default function App(){
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 p-6">
          <main className="lg:col-span-1 space-y-8">
            <section>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {styleTiles.map(s=> (
                  <button key={s.key} onClick={()=>setSel(v=>({...v, style: s.key }))} className={`group overflow-hidden rounded-2xl shadow hover:shadow-lg transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 ring-offset-2 ${sel.style===s.key?'ring-2 ring-indigo-500':''}`} aria-label={`Выбрать стиль ${s.key}`}>
              ...
              <section>
                ...
                {models.length ? (
                  <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
                    {models.map(m=> (
                      <DoorCard key={m.model} item={m} selected={sel.model===m.model} onSelect={()=>setSel(v=>({...v, model: m.model }))} />
                    ))}
                  </div>
          ...
          <section className="lg:col-span-1">
            <div className="sticky top-6">
              <StickyPreview item={sel.model ? { model: sel.model, sku_1c: price?.sku_1c } : null} />
            </div>
          </section>
        ...
        <div className="max-w-3xl mx-auto p-6 space-y-8">
          ...
          <section className="bg-white rounded-2xl shadow p-4">
            ...
            <form onSubmit={importPrice} className="space-y-3">
              ...
              <label className="text-sm block">Mapping JSON (опционально)
                <textarea value={mappingText} onChange={e=>setMappingText(e.target.value)} className="w-full border rounded px-3 py-2 font-mono text-sm min-h-[160px]" />
              </label>
              ...
              <div className="mt-3 text-sm">
                {importInfo.ok && importInfo.status===200 && (
                  <div className="rounded-lg border p-3 bg-green-50">
                    ...
                    <pre className="bg-white border mt-2 p-2 rounded overflow-auto">{JSON.stringify(importInfo.body, null, 2)}</pre>
                  </div>
                )}
                {!importInfo.ok && importInfo.status===409 && (
                  <div className="rounded-lg border p-3 bg-yellow-50">
                    ...
                    <pre className="bg-white border mt-2 p-2 rounded overflow-auto">{JSON.stringify(importInfo.body, null, 2)}</pre>
                    {(importInfo.body?.report_csv || importInfo.body?.conflicts_report) && (
                      <a className="underline" href={importInfo.body.report_csv || importInfo.body.conflicts_report} target="_blank">Открыть CSV-отчёт</a>
                    )}
                  </div>
                )}
                {!importInfo.ok && importInfo.status!==409 && (
          ...
          <section className="bg-white rounded-2xl shadow p-4">
            ...
            <form onSubmit={uploadMedia} className="space-y-3">
              <label className="text-sm block">Model
                <input value={modelForPhoto} onChange={e=>setModelForPhoto(e.target.value)} placeholder="например: PO Base 1/1" className="w-full border rounded px-3 py-2" />
              </label>
              <input type="file" name="media" accept="image/jpeg,image/png,image/webp" multiple className="block" />
              <div className="text-xs text-gray-500">Имя файла: <code>encodeURIComponent(model).ext</code>; папка: <code>public/assets/doors/</code></div>
              <button className="px-4 py-2 rounded-xl bg-black text-white" type="submit">Загрузить</button>
            </form>
              ...
              <div className="mt-3 text-sm">
                {mediaInfo.ok ? (
                  <div className="rounded-lg border p-3 bg-green-50">
                    ...
                    <pre className="bg-white border mt-2 p-2 rounded overflow-auto">{JSON.stringify(mediaInfo.body, null, 2)}</pre>
                    {mediaInfo.body?.files?.[0]?.url && (
                      <a className="underline" href={mediaInfo.body.files[0].url} target="_blank">Открыть {mediaInfo.body.files[0].filename}</a>
                    )}
                  </div>
                ) : (
                  <div className="rounded-lg border p-3 bg-red-50">
      ...
      {/* SSR smoke marker for CI */}
      <div data-smoke="compat-active" hidden />
    </div>
  )
}
      ...
      <div className="p-4 flex flex-col gap-3">
        <div className="aspect-[3/4] w-full overflow-hidden rounded-xl bg-gray-50">
          {src ? (
            ...
            <img src={src} alt={item.model} className="h-full w-full object-cover" onError={handleError} />
          ) : (
